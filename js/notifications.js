/* =========================================================
   FitPulse - Sistema de Notificaciones (PWA compatible)
...
const Notificaciones = {
  ...
  async _send(titulo, body) {
    try {
      const reg = await navigator.serviceWorker.ready;
      await reg.showNotification(`FitPulse - ${titulo}`, {
        body,
        icon: './icon-192.png',
        badge: './icon-192.png',
      ...
      try {
        new Notification(`FitPulse - ${titulo}`, { body, tag: `nutritrack-${titulo}` });
      } catch (e2) { /* silent */ }
    }
  },
